...
volatile int16_t g_base_speed = 23;
...
void Tracking_Process(void)
{
    ...
    /* The sensor is active-low: convert it to 1 = black line. */
    d8 = (uint8_t)!data_1.D8;
    d7 = (uint8_t)!data_1.D7;
    d6 = (uint8_t)!data_1.D6;
    d5 = (uint8_t)!data_1.D5;
    d4 = (uint8_t)!data_1.D4;
    d3 = (uint8_t)!data_1.D3;
    d2 = (uint8_t)!data_1.D2;
    d1 = (uint8_t)!data_1.D1;

    black_mask = (uint8_t)((d8 << 7U) | (d7 << 6U) |
                           (d6 << 5U) | (d5 << 4U) |
                           (d4 << 3U) | (d3 << 2U) |
                           (d2 << 1U) | d1);
    black_count = (uint8_t)(d1 + d2 + d3 + d4 + d5 + d6 + d7 + d8);
    ...
    if (((black_count == 4U) || (black_count == 3U)) &&
        ((d3 && d4) || (d4 && d5) || (d2 && d3))) {
        if (able_stop) {
            able_stop = 0;
            Motor_Brake();
